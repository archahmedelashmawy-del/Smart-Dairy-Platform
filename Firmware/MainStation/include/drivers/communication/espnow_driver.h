...
public:
    ...
    using ReceiveCallback = void (*)(const ReceivedPacket&);
    ...
    ErrorCode sendPacket(const uint8_t* mac, const SmartPacket& packet);
    ErrorCode broadcast(const SmartPacket& packet);
...
private:
    ...
    // Unified helper callbacks matching CPP
    static void onReceiveInternal(const esp_now_recv_info* info, const uint8_t* data, int len);
    static void onSendInternal(const uint8_t* mac, esp_now_send_status_t status);

    void handleReceive(const esp_now_recv_info* info, const uint8_t* data, int len);
    void handleSend(const uint8_t* mac, esp_now_send_status_t status);

    static ErrorCode validatePacket(const SmartPacket& packet, int len);
